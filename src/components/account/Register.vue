  ...
  methods: {
    ...
    register() {
      if (Object.values(this.user).indexOf(null)) {
        if (this.verifyNick() && this.verifyEmail() && this.validatePassword()) {
          userRegister(this.user).then(r => {
            if (!r.data.code) {
              ElMessage({message: '注册成功', title: 'Success', type: 'success'});
              this.router().push({name:'login'})
            } else {
              ElMessage({message: '注册异常', title: 'Error', type: 'error'})
            }
          }).catch(e=>{
            ElMessage({message: e.message, title: 'Error', type: 'error'})
          })
        ...
        ElMessage({message: '请先填写信息', title: 'Error', type: 'error'})
      }
    },
    verifyNick() {
